...
- Dish scores mix in `combineRecipeTaste`: recipe-relative loudness (blend from quieted `score × (score/peak)^⅓` up to full score as ratio→1; no 85% cliff), then **per-ingredient** blend of linear `share×loud` and punch `loud×share^(1/p)` (p=4). Punch weight = share smoothstep (midpoint ≈1.5%, loudness ~4→7) × intensity smoothstep (~5.25→10). Crowding on a dimension does not change an ingredient’s punch term; mid notes (ketchup ≈7) stay partial; sugar/chili ≈10 punch a bit less than before; **table salt leaf ≈ 12** so a spoon still seasons a bowl. Sum → linear gain (1.75×). Cap at strongest in-ingredient leaf, then clamp dish display to 0–10. Bland food stays low. Pure ingredients return the catalog vector (salt may show 12). Spicy is chili heat only: ginger/garlic/mustard/Sichuan peppercorn = 0, black pepper ≈ 0.2, freshly cracked ≈ 0.5 via `mix.scale`, Thai chili = 10. A usual 10 is the most intense culinary form (sugar, Thai chili); salt is the exception at ≈12; lemon/lime fruit ≈ 9 sour, juice ≈ 9.5.
- After recipes are collected and names canonicalized, if any **in** line has a missing / “to taste” / “as needed” amount, Gemini `estimateMissingAmounts` fills those amounts. It sees every recipe’s ingredients (measured lines as stated, unknowns marked). When some recipes measured that ingredient, match them (scaled to recipe size). When **every** recipe omitted it, estimate from the dish, cuisine, and the other ingredients in that recipe. Code converts the returned amount+unit like a normal measurement; a crazy guess is clamped toward sibling measured volumes when those exist. LLM failure keeps the code volume fallback. Mix stays engine-only.
- Classify input first (dish / ingredient / reject). Reject brands and random text. If the name is both a grocery and a cooked dish (lasagna, spaghetti), classify as dish and run the recipe path — prefer dish whenever people cook or order it. **Ingredient Turso wins** when classify says ingredient (and when resolving names inside a recipe). Reuse cache only gates the **dish** average for what the user typed. Unmatched trailing junk (`]`, `!!!`) is stripped from the typed name first; the dish is not renamed.
- Unknown names: try chemistry (FAO/INFOODS first, then UmamiDB / Phenol-Explorer / Dr. Duke / FooDB / USDA). One Gemini shortlist check must confirm the food against the dish (chili in 辣子鸡 is hot chili, not sweet chili sauce). Deterministic compound mixer, then Gemini may calibrate dimensions that have evidence (and chili heat on chili-named foods when labs hit sweet pepper). Acid-process foods (fermented/pickled/yogurt/vinegar-class names) need organic-acid evidence — sodium-only lab rows are incomplete and fall through. If no source matched that **exact** grocery name (or the leaf is incomplete), Gemini `estimateLeafTaste` scores a mouthful (`source: "llm"`, low confidence). Calibrate and estimate rank the mouthful against known catalog anchors (sugar, brown sugar, mirin, soy, salt, cheese, garlic, ginger, …) and may carefully raise or lower so relative scale stays sane (mirin ≈ 5–7 sweet, never ≥ crystalline sugar; plain cheese ≈ 5 salty, not sauce-level). Do not collapse names (thai chili ≠ chili; soft shell crab ≠ crab). If estimate is missing or fails, run the same full recipe search and persist the mix as an **ingredient**. Nested recipe-ingredients get that same search. Existing catalog rows are never overwritten (`INSERT OR IGNORE`).
- Translate ingredient names to English (or a well-known romanized native name when English is awkward) before scoring or showing them in the UI. Gemini `canonicalizeIngredientNames` takes the extracted names, the ingredient catalog, and the dish's culinary origin. Ambiguous generics must become the cuisine-typical grocery even when the catalog only has the dictionary form (Chinese dish + 香肠 / sausage → chinese sausage or lap cheong, not plain sausage). Product-family names whose members taste different must become the cuisine-typical grocery for this dish. Already-specific names stay themselves (italian sausage stays italian sausage). Catalog match is only at the same specificity — never copy a catalog hypernym. Never collapse process forms into a catalog parent (kimchi ≠ cabbage; pickle ≠ cucumber). A grocery named in the dish title must stay itself. Recipe extract also returns prep mix knobs (`mix.intensity` / `mix.scale` / optional `mix.why`) using culinary common sense — no prep enum in code. `mix.intensity` is how much of the listed amount contributes to the final served dish. When intensity ≠ 1, `mix.why` is a 1–2 word label (marinade, evaporated, absorbed) shown on the ingredient hover as `contributes: N% · why`. Drained frying oil, pasta water, blanching water, and evaporated/absorbed cooking water are intensity 0 and do not dilute the dish. Code also zeroes bulk neutral fry oil and evaporated/absorbed water (with a small floor for stock/broth/wine) when extract omits intensity.
...
- `lib/engine/fill-amounts.ts` — apply Gemini missing-amount estimates; clamp toward sibling measurements
- `lib/engine/ambiguous-seasoning.ts` — name classes used for code to-taste volume fallback if Gemini amount-fill fails
