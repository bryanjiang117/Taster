...
- Dish scores mix in `combineRecipeTaste`: recipe-relative loudness for non-spicy dims (peer scores stay; traces quiet as `score × √(score/peak)`), then a smooth blend of linear volume×score and p-norm punch-through (p=4; weight ramps with loud seasoning share, midpoint ≈2.5%), linear gain (1.75×). **Spicy** skips relative loudness and always p-norms. Cap at strongest in-ingredient leaf. Bland food stays low. Pure ingredients return the catalog 0–10 vector (no dilution). Spicy is chili heat only: ginger/garlic/mustard/Sichuan peppercorn = 0, black pepper ≈ 0.2, freshly cracked ≈ 0.5 via `mix.scale`, Thai chili = 10. A 10 is the most intense culinary form (salt, sugar, Thai chili); lemon/lime fruit ≈ 9 sour, juice ≈ 9.5.
