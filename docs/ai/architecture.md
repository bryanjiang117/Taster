...
```
input
  → tidy unmatched trailing punctuation ("]", "!!!") without renaming the dish
...
Hard cases (ambiguous origin, fermented/compound ingredients, leaf calibration, missing recipe amounts) retry on `gemini-3.6-flash`. Model IDs live in `lib/engine/models.ts` (`FAST_MODEL` / `SMART_MODEL`).
...
| Start limits | `rate-limit.ts` | Turso `rate_limits`; `VERCEL_ENV=production` only | no scoring |
...
Per-visitor start caps in the same Turso database, table `rate_limits`. On only when `VERCEL_ENV=production` (Vercel production). Local `npm run dev` / `next start` and Vercel preview skip the check. Counted when a tasting is about to start (valid dish name on `POST /api/profile`), not when it finishes. Stop, reject, and error still consumed a start. Empty names are not counted. `GET /api/stats` is not limited.

Fixed clock buckets from epoch: 5 per minute, 15 per hour, 25 per day. Visitor key is a hash of the first `x-forwarded-for` hop (`x-real-ip` then `"unknown"` if missing). Over any cap returns 429 JSON `{ error }` with a short wait message; the UI already shows that. A blocked start does not increment any bucket. Tests: `lib/engine/rate-limit.test.ts` (in-memory `file:` libSQL).

```sql
CREATE TABLE IF NOT EXISTS rate_limits (
  visitor TEXT NOT NULL,
  window TEXT NOT NULL,   -- minute | hour | day
  start INTEGER NOT NULL, -- bucket start, unix ms
